...
public class DartResolver implements ResolveCache.AbstractResolver<DartReference, List<? extends PsiElement>> {
  ...
  public List<? extends PsiElement> resolve(@Nonnull DartReference reference, boolean incompleteCode) {
    ...
    if (reference instanceof DartParameterNameReferenceExpression) {
      ...
      final DartFormalParameterList parameters = PsiTreeUtil.getChildOfType(target != null ? target.getParent() : null,
    ...
    if (references != null && references.length == 2) {
      // prefix
      final List<DartComponentName> names = DartResolveUtil.findComponentsInLibraryByPrefix(reference, references[0].getCanonicalText(),
      ...
      if (dartClass != null) {
        ...
        final DartComponent subComponent = leftReference instanceof DartType ? dartClass.findNamedConstructor(name) : filterAccess
          (reference, dartClass.findMembersByName(name));
        return toResult(subComponent == null ? null : subComponent.getComponentName());
      }
      // prefix
      final List<DartComponentName> names = DartResolveUtil.findComponentsInLibraryByPrefix(reference, leftReference.getCanonicalText(),
  ...
  private static DartComponent filterAccess(PsiElement element, List<DartComponent> components) {
    ...
    return ContainerUtil.find(components, new consulo.util.lang.function.Condition<DartComponent>() {
      @Override
  ...
  private static List<? extends PsiElement> resolveSimpleReference(@Nonnull DartReference reference) {
    ...
    final boolean isSimpleConstructor = parent instanceof DartType && superParent instanceof DartNewExpression && ((DartNewExpression)
      superParent).getReferenceExpression() == null;
