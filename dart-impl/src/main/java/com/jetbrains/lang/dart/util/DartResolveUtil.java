...
public class DartResolveUtil {
  ...
  public static boolean checkParametersType(DartFormalParameterList list, DartClass... classes) {
    ...
    for (int size = namedFormalParameters.getDefaultFormalNamedParameterList().size(); i - normalFormalParameterList.size() < size; ++i) {
      final DartDefaultFormalNamedParameter defaultFormalParameter = namedFormalParameters.getDefaultFormalNamedParameterList().get(i -
  ...
  private static boolean canAssign(@Nullable final DartClass baseClass, @Nullable DartClass aClass) {
    ...
    final Ref<Boolean> result = consulo.util.lang.ref.Ref.create(false);
  ...
  public static List<DartClass> findClassesByParent(@Nonnull final DartClass superClass, @Nullable PsiElement context) {
    ...
    return ContainerUtil.filter(classesInFile, new consulo.util.lang.function.Condition<DartClass>() {
      @Override
  ...
  private static boolean checkInheritanceBySuperAndImplementationList(DartClass dartClass, DartClass superCandidate) {
    if (typeResolvesToClass(dartClass.getSuperClass(), superCandidate)) {
      return true;
    }
    for (DartType dartType : getImplementsAndMixinsList(dartClass)) {
      if (typeResolvesToClass(dartType, superCandidate)) {
        return true;
      }
    }
    return false;
  ...
  public static List<DartComponentName> findComponentsInLibraryByPrefix(PsiElement contextToSearch,
    ...
    if (virtualFile != null) {
      final List<DartComponentName> result = new ArrayList<DartComponentName>();
      processTopLevelDeclarations(contextToSearch, new DartResolveProcessor(result, componentName), virtualFile, componentName);
      return result;
    }
    return Collections.emptyList();
  }
  ...
  public static VirtualFile getImportedFileByImportPrefix(final @Nonnull PsiElement context, final @Nonnull String prefix) {
    ...
    for (VirtualFile virtualFile : virtualFiles) {
      for (DartImportOrExportInfo importOrExportInfo : DartImportAndExportIndex.getImportAndExportInfos(project, virtualFile)) {
        if (importOrExportInfo.getKind() != Kind.Import) {
          continue;
        }
  ...
  private static boolean processTopLevelDeclarationsImpl(final @Nonnull PsiElement context,
                                                         final @Nonnull DartPsiScopeProcessor processor,
                                                         final @Nullable VirtualFile virtualFile,
                                                         final @Nullable Set<String> fileNames,
                                                         final @Nonnull Set<VirtualFile> processedFiles) {
  ...
  private static boolean processTopLevelDeclarationsImpl(final @Nonnull PsiElement context,
    ...
    for (DartImportOrExportInfo importOrExportInfo : DartImportAndExportIndex.getImportAndExportInfos(context.getProject(), virtualFile)) {
      if (processingLibraryWhereContextElementLocated && importOrExportInfo.getKind() == Kind.Export) {
        continue;
      }
      if (!processingLibraryWhereContextElementLocated && importOrExportInfo.getKind() == Kind.Import) {
        continue;
      }
  ...
  private static VirtualFile getImportedFile(final @Nonnull Project project, final @Nonnull VirtualFile contextFile,
    ...
    return parent == null ? null : VirtualFileUtil.findRelativeFile(importText, parent);
  }
  ...
  public static VirtualFile findRelativeFile(@Nonnull VirtualFile file, String path) {
    ...
    if (parent == null) {
      return VirtualFileManager.getInstance().findFileByUrl(VirtualFileUtil.pathToUrl(path));
    }
    return VirtualFileUtil.findRelativeFile(path, parent);
  }
  ...
  public static List<VirtualFile> findLibrary(final PsiFile context, GlobalSearchScope scope) {
    ...
    return ContainerUtil.filter(DartSourceIndex.findLibraries(context, context.getName(), scope),
                                new consulo.util.lang.function.Condition<VirtualFile>() {
                                  @Override
  ...
  private static List<String> findSourcePathByFileName(Project project, VirtualFile virtualFile, final String fileName) {
    return ContainerUtil.filter(DartPathIndex.getPaths(project, virtualFile), new consulo.util.lang.function.Condition<String>() {
      @Override
  ...
  public static boolean isLibraryRoot(PsiFile psiFile) {
    for (PsiElement root : findDartRoots(psiFile)) {
      if (PsiTreeUtil.getChildOfType(root, DartPartOfStatement.class) != null) {
        return false;
      }
    }
    return true;
  ...
  public static List<DartComponent> findNamedSubComponents(boolean unique, @Nonnull DartClass... rootDartClasses) {
    final List<DartComponent> unfilteredResult =
      findSubComponents(new Function<DartClass, List<DartComponent>>() {
        @Override
        public List<DartComponent> apply(DartClass dartClass) {
  ...
  public static List<DartOperator> findOperators(AbstractDartPsiClass dartPsiClass) {
    return findSubComponents(new Function<DartClass, List<DartOperator>>() {
      @Override
      public List<DartOperator> apply(DartClass dartClass) {
  ...
  @Nonnull
  public static <T> List<T> findSubComponents(final Function<DartClass, List<T>> fun,
                                              @Nonnull DartClass... rootDartClasses) {
    final List<T> unfilteredResult = new ArrayList<T>();
    processSuperClasses(new PsiElementProcessor<DartClass>() {
      @Override
      public boolean execute(@Nonnull DartClass dartClass) {
        unfilteredResult.addAll(fun.apply(dartClass));
        return true;
      }
    }, rootDartClasses);
    return unfilteredResult;
  ...
  public static List<DartComponentName> getComponentNames(List<? extends DartComponent> fields) {
    return ContainerUtil.filter(ContainerUtil.map(fields,
                                                  (Function<DartComponent, DartComponentName>)component -> component.getComponentName()), consulo.util.lang.function.Condition.NOT_NULL);
  }
  ...
  @Nonnull
  public static DartClassResolveResult getDartClassResolveResult(@Nullable PsiElement element,
                                                                 @Nonnull DartGenericSpecialization specialization) {
    if (element == null) {
    ...
    final DartVarInit varInit = PsiTreeUtil.getChildOfType(element instanceof DartVarDeclarationListPart ? element : parentElement,
                                                           DartVarInit.class);
  ...
  public static DartComponentName findParameterByName(@Nullable DartFormalParameterList parameterList, final String name) {
    ...
    for (DartDefaultFormalNamedParameter namedParameter : namedFormalParameters.getDefaultFormalNamedParameterList()) {
      ...
      if (parameterName != null && name.equals(parameterName.getText())) {
  ...
  private static PsiElement findParameter(@Nullable PsiElement element, int index) {
    ...
    final DartNamedFormalParameters namedFormalParameters = parameterList.getNamedFormalParameters();
    return namedFormalParameters == null ? null : namedFormalParameters.getDefaultFormalNamedParameterList()
                                                                       .get(index - normalParameterSize);
  }
  ...
  private static DartClassResolveResult resolveForInPartClass(DartForInPart forInPart) {
    ...
    final DartClassResolveResult iteratorResult = dartClass == null ? DartClassResolveResult.EMPTY : getDartClassResolveResult(dartClass
                                                                                                                                 .findMemberByName(
                                                                                                                                   "iterator"),
                                                                                                                               classResolveResult
                                                                                                                                 .getSpecialization());
  ...
  public static int getArgumentIndex(PsiElement place) {
    ...
    if (place == argumentList) {
      ...
      final DartFunctionDescription functionDescription = DartFunctionDescription.tryGetDescription((DartCallExpression)argumentList
        .getParent());
      // the last one
  ...
  public static boolean aloneOrFirstInChain(DartReference reference) {
    return PsiTreeUtil.getChildrenOfType(reference, DartReference.class) == null && getLeftReference(reference) == null && getLeftReference
  ...
  public static ResolveResult[] toCandidateInfoArray(@Nullable List<? extends PsiElement> elements) {
    ...
    elements = ContainerUtil.filter(elements, new consulo.util.lang.function.Condition<PsiElement>() {
      @Override
