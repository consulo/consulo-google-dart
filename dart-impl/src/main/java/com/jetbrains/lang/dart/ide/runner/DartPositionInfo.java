...
public class DartPositionInfo {
  ...
  public DartPositionInfo(final @Nonnull Type type,
  ...
  public static DartPositionInfo parsePositionInfo(final @Nonnull String text) {
    ...
    if (pathStartAndEnd == null) {
      pathStartAndEnd = parseUrlStartAndEnd(text, "dart:");
    }
    if (pathStartAndEnd == null) {
      pathStartAndEnd = parseUrlStartAndEnd(text, "file:");
    }
    if (pathStartAndEnd == null) {
      pathStartAndEnd = parseDartLibUrlStartAndEnd(text);
    }
    if (pathStartAndEnd == null) {
      return null;
    }
    ...
    if (lineAndColumn == null) {
      lineAndColumn = parseLineAndColumnInTextFormat(tail);
    }
    ...
    final int pathStartIndexInUrl = type == Type.FILE ? colonIndexInUrl + 1 + getPathStartIndex(url.substring(colonIndexInUrl + 1)) :
      colonIndexInUrl + 1;
  ...
  private static Couple<Integer> parseDartLibUrlStartAndEnd(final String text) {
    ...
    while (text.length() > index && Character.isLetter(text.charAt(index))) {
      index++;
    }
