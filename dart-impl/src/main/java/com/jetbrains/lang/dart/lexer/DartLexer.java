...
public class DartLexer extends MergingLexerAdapterBase {
   ...
   * collapses sequence like <code>{MULTI_LINE_(DOC_)COMMENT_START MULTI_LINE_COMMENT_BODY* MULTI_LINE_COMMENT_END}</code> into a single
   * <code>DartTokenTypesSets.MULTI_LINE_(DOC_)COMMENT</code>.
    ...
    public IElementType merge(final IElementType firstTokenType, final Lexer originalLexer) {
      if (firstTokenType == REGULAR_STRING_PART || firstTokenType == WHITE_SPACE) {
        ...
        while (true) {
          ...
          if (nextTokenType != firstTokenType) {
            break;
          }
        ...
        while (true) {
          ...
          if (nextTokenType == null) {
            break; // EOF reached, multi-line comment is not closed
          }

          originalLexer.advance();
          if (nextTokenType == MULTI_LINE_COMMENT_END) {
            break;
          }
        ...
        return firstTokenType == MULTI_LINE_DOC_COMMENT_START ? MULTI_LINE_DOC_COMMENT : MULTI_LINE_COMMENT;
      }
