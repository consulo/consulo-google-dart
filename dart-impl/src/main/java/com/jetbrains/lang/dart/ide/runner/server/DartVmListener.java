      ...
      */
		...
		if(PausedReason.breakpoint == reason && !isStepping && topFrame != null)
		{
			...
			if(breakpoint != null)
			{
				...
				final boolean suspend = myDebugProcess.getSession().breakpointReached(breakpoint, evaluateExpression(isolate, topFrame,
						breakpoint.getLogExpressionObject()), new DartSuspendContext(myDebugProcess, frames));
				if(!suspend)
				{
					resume(isolate);
				}
