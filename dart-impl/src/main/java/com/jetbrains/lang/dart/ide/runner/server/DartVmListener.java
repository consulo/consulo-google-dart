...
public class DartVmListener implements VmListener {
  ...
  public void debuggerPaused(final PausedReason reason, final VmIsolate isolate, final List<VmCallFrame> frames, final VmValue exception,
    ...
    if (PausedReason.breakpoint == reason && !isStepping && topFrame != null) {
      ...
      if (breakpoint != null) {
        ...
        final boolean suspend = myDebugProcess.getSession().breakpointReached(breakpoint,
                                                                              evaluateExpression(isolate, topFrame,
                                                                                                 breakpoint.getLogExpressionObject()),
  ...
  @Nullable
  private String evaluateExpression(final @Nonnull VmIsolate isolate,
                                    final @Nonnull VmCallFrame topFrame,
    ...
    final consulo.util.lang.ref.Ref<String> evalResult = new Ref<String>();
  ...
  private void firstIsolateInit(final VmIsolate isolate) {
    ...
    try {
      myDebugProcess.getVmConnection().setPauseOnExceptionSync(isolate, VmConnection.BreakOnExceptionsType.unhandled); // todo add an option
      // for BreakOnExceptionsType
    }
    catch (IOException e) {
      LOG.error(e);
    }
