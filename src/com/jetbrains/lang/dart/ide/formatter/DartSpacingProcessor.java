...
import static com.jetbrains.lang.dart.DartTokenTypesSets.DART_FILE;
...
{
	private static final TokenSet TOKENS_WITH_SPACE_AFTER = TokenSet.create(VAR, FINAL, STATIC, EXTERNAL, ABSTRACT, GET, SET, FACTORY, OPERATOR,
			PART, EXPORT, DEFERRED, AS, SHOW, HIDE, RETURN_TYPE);
	...
	public Spacing getSpacing(final Block child1, final Block child2)
	{
		...
		if(type1 == LPAREN || type2 == RPAREN)
		{
			...
			{
				final boolean newLineNeeded = type1 == LPAREN ? mySettings.METHOD_PARAMETERS_LPAREN_ON_NEXT_LINE : mySettings
						.METHOD_PARAMETERS_RPAREN_ON_NEXT_LINE;
				return addSingleSpaceIf(mySettings.SPACE_WITHIN_METHOD_PARENTHESES, newLineNeeded);
			}
			else if(elementType == ARGUMENTS)
			{
				final boolean newLineNeeded = type1 == LPAREN ? mySettings.CALL_PARAMETERS_LPAREN_ON_NEXT_LINE : mySettings
			...
			{
				final boolean newLineNeeded = type1 == LPAREN ? mySettings.PARENTHESES_EXPRESSION_LPAREN_WRAP : mySettings
